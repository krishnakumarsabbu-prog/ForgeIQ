...
def create_harness_version(harness_id: str, body: VersionBody):
    ...
    new_version = HarnessVersion(
        ...
        loop_ids=body.loop_ids if body.loop_ids is not None else (latest.loop_ids if latest else h.loop_ids),
        agent_ids=body.agent_ids if body.agent_ids is not None else (latest.agent_ids if latest else h.agent_ids),
        skill_ids=body.skill_ids if body.skill_ids is not None else (latest.skill_ids if latest else h.skill_ids),
        tool_ids=body.tool_ids if body.tool_ids is not None else (latest.tool_ids if latest else h.tool_ids),
        model_config_ids=body.model_config_ids if body.model_config_ids is not None else (latest.model_config_ids if latest else h.model_config_ids),
        policy_ids=body.policy_ids if body.policy_ids is not None else (latest.policy_ids if latest else h.policy_ids),
        environment=body.environment if body.environment is not None else (latest.environment if latest else h.environment),
        cost_limit_cents=body.cost_limit_cents if body.cost_limit_cents is not None else (latest.cost_limit_cents if latest else h.cost_limit_cents),
        time_limit_seconds=body.time_limit_seconds if body.time_limit_seconds is not None else (latest.time_limit_seconds if latest else h.time_limit_seconds),
...
def create_template_version(template_id: str, body: TemplateVersionBody):
    ...
    new_v = HarnessTemplateVersion(
        ...
        is_default=False,
        is_immutable=False,
        mandatory_steps=body.mandatory_steps if body.mandatory_steps is not None else (latest.mandatory_steps if latest else t.mandatory_steps),
        optional_steps=body.optional_steps if body.optional_steps is not None else (latest.optional_steps if latest else t.optional_steps),
        configurable=body.configurable if body.configurable is not None else (latest.configurable if latest else t.configurable),
        ...
        tenant_override_forbidden=body.tenant_override_forbidden if body.tenant_override_forbidden is not None else (latest.tenant_override_forbidden if latest else t.tenant_override_forbidden),
        default_config=body.default_config if body.default_config is not None else (latest.default_config if latest else t.default_config),
        changelog=body.changelog,
        created_at=utc_now(),
    )
...
def get_template_inheritance(template_id: str):
    ...
    return {
        ...
        "governance_enforced": all(
            any(step in s["mandatory_steps"] for s in chain)
