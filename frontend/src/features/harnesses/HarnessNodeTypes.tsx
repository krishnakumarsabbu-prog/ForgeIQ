...
function HarnessNodeBase({ data, selected }: NodeProps) {
  const d = data as unknown as HarnessNodeData
  ...
  return (
    <div
      className={`relative rounded-lg border-2 bg-white shadow-sm transition-shadow min-w-[180px] ${
        selected ? 'border-forgeiq-500 shadow-md ring-2 ring-forgeiq-200' : hasErrors ? 'border-red-400' : cfg.border
      }`}
    >
...
export const harnessNodeTypes = { harnessNode: memo(HarnessNodeBase) } as unknown as Record<string, typeof HarnessNodeBase>

export const NODE_PALETTE: { type: string; label: string; icon: typeof Bot; color: string; bg: string }[] = [
